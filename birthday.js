...
function showFinale(immediate = false){
  ...
  if (immediate){
    ...
    gsap.set('.finale__veil', { opacity: 1 });
    gsap.set(finaleCard, { opacity: 1, y: 0, scale: 1 });
    return;
  }

  gsap.killTweensOf([finale, finaleCard, '.finale__veil']);
  gsap.to(finale, { autoAlpha: 1, duration: .5, ease: 'power2.out' });
  gsap.to('.finale__veil', { opacity: 1, duration: .75, ease: 'power2.out' });
  gsap.to(finaleCard, { opacity: 1, y: 0, scale: 1, duration: .9, delay: .18, ease: 'back.out(1.25)' });
}

function hideFinale(){
  closePanels();
  gsap.killTweensOf([finale, finaleCard, '.finale__veil']);
  gsap.to(finaleCard, { opacity: 0, y: 24, scale: .97, duration: .25, ease: 'power2.in' });
  gsap.to('.finale__veil', { opacity: 0, duration: .25, ease: 'power2.in' });
  gsap.to(finale, {
    autoAlpha: 0, duration: .3, delay: .1, ease: 'power2.in',
    onComplete: () => finale.classList.remove('is-visible')
  });
...
function openPanel(panel){
  ...
  gsap.fromTo(panel, { opacity: 0, y: 28, scale: .98 }, {
...
function setupFinale(){
  ...
  gsap.set('.finale__veil', { opacity: 0 });
  gsap.set(finaleCard, { opacity: 0, y: 24, scale: .97 });

  openLetter.addEventListener('click', () => openPanel(letterPanel));
  openReasons.addEventListener('click', () => openPanel(memoryPanel));
  closeLetter.addEventListener('click', closePanels);
  closeReasons.addEventListener('click', closePanels);
  replayFinale.addEventListener('click', resetAll);
  ...
  finale.addEventListener('click', (e) => {
    if (e.target === finale || e.target.classList.contains('finale__veil')) closePanels();
  });
...
function treeFrame(now){
  ...
  if (!replayArmed && t >= T.done + 1.0){ replayArmed = true; armReplay(); showFinale(); }
